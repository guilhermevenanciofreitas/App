...
import { FaFileImport, FaTransgender } from 'react-icons/fa';
...
class FinanceBankAccounts extends React.Component {
  ...
  onImport = () => {
    this.setState({loading: true}, async () => {
      try {
        await new Service().Post('logistic/cte/import', this.state.request).then((result) => this.setState({...result.data})).finally(() => this.setState({loading: false}))
      } catch (error) {
        toast.error(error.message)
      }
    })
    ...
    return (
      <>
        ...
        <PageContent>
          ...
          <Button appearance='primary' color='blue' startIcon={<FaFileImport />} onClick={this.onImport}>&nbsp;Importar</Button>
...
class Page extends React.Component {
  ...
  render = () => {
    return (
      <Panel header={<CustomBreadcrumb menu={'Logística'} title={'Conhecimento de transportes'} />}>
        <FinanceBankAccounts />
      </Panel>
    )
  }
